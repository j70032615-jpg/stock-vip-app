...
# 1. 頁面配置
st.set_page_config(page_title="股票心法 VIP 終極版", layout="wide", page_icon="📈")
...
if not st.session_state['logged_in']:
    ...
    if st.session_state['tab'] == "九大心法":
        ...
        with st.expander("一. 畫趨勢線 (大方向與買點)", expanded=True):
            st.markdown("""
        ...
        with st.expander("三. 用箱形突破找加碼點跟出場點"):
            ...
            **B. 在箱型中間畫一條線**
            1. 畫好箱型，算箱型頭部到底有多少價差 假設頭部100塊 底部90塊，取中間值 100-90=10 再除2=5 也就是95塊劃一條線
            ...
            5. 相形的波段箭頭假設用短線15分鐘線來看一波箭頭是400點 未來下跌一波也可能是400點 如果有看到跌2波 要看盤整後是否反彈回升
            """)
        ...
        with st.expander("五. 懶人穩勝法 & 六. 裸K操作"):
            st.markdown("""
            **懶人穩勝法：**
            1. 當K棒下跌到周線的20MA之下時 空頭機率極高可以先賣出
            2. 如果能順利站上周線的20MA以及60MA 都不跌破的話 基本上就可以80%認定是站回多頭了
            3. 用投資先生KDJ指標看月線ETF交叉線從20下方往上走做多 從80往下交叉準備出場
            
            **裸K看法：**
            - 底部盤整都是短K棒 主力默默吃貨. 吃越久的短K棒+上爆量上漲 就越有機會漲100%
            - 高勝率裸K就是找超長線 周 月 半年線 年線來找跌轉漲的大K棒來做
            """)

        with st.expander("七. 資金分配法 & 八. 精準支撐壓力"):
            st.markdown("""
            **資金分配：**
            1. 60%資金挑2檔高股息ETF；30%挑1檔波動型ETF；10%選短線個股。
            2. 每獲利20%就出場10%資金；每跌20%就進場之前獲利出場的資金20%。
            3. 增強心裡波動法：用4小時K線來看，看不到紅K棒心裡就比較不會有波動。
        ...
        st.error("避免操作錯誤，請對照以下準則：")
        st.markdown("""
        1. **該不該加碼**：看1分線或5分線的200日均線有沒有
